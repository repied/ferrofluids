...
void loop() {
    ...
    switch (curState){
        ...
        case SINUS_STATE: // filed oscillate with slowly varying frequency
            ...
            fieldSmall01 = (1 + sin(tau * freq * t)) / 2;
            fieldBig01 = (1 - sin(tau * freq * t)) / 2;
            if ((t-tInit) > secBeforSleep / 2. ) { // half of the time same dir
                dirBig = LOW;
            } else {
                dirBig = HIGH;
            }
...
int nextStateFromSinus(float ir01, float t) {
    if (ir01 > irThres01) {
        return jumpToState(PROXI_STATE);
    }
    else { // no ir
